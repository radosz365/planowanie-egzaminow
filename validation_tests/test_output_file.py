...
def test_no_lecturer_conflicts():
    ...
    assert all(conflicts <= 1), 'A lecturer has more than one exam in the same time slot.'
...
def test_no_classroom_conflicts():
    ...
    assert all(conflicts <= 1), 'A classroom has more than one exam in the same time slot.'
